...
class CleanReportGenerator:
    ...
    def get_summary(self) -> str:
        ...
        if self.stats['empty_cols_dropped'] > 0:
            lines.append(f"🗑️ Colonnes supprimées (vides): {self.stats['empty_cols_dropped']}")

        # Doublons
        if self.stats['duplicates_removed'] > 0:
            lines.append(f"🔄 Doublons supprimés: {self.stats['duplicates_removed']}")

        # Types convertis
        if self.stats['types_converted']:
            conv_details = ", ".join([f"{k} ({v})" for k, v in self.stats['types_converted'].items()])
            lines.append(f"🎨 Types convertis: [{conv_details}]")

        # Valeurs manquantes comblées
        if self.stats['missing_filled']:
            filled_cols = list(self.stats['missing_filled'].keys())
            lines.append(f"💧 Valeurs manquantes comblées dans: {', '.join(filled_cols)}")

        # Outliers corrigés
        if self.stats['outliers_corrected']:
            outlier_details = ", ".join([f"{k} ({v})" for k, v in self.stats['outliers_corrected'].items()])
            lines.append(f"📈 Outliers corrigés (IQR): [{outlier_details}]")

        # Nettoyage espaces
        if self.stats['whitespace_cleaned'] > 0:
            lines.append(f"🧼 Colonnes avec espaces nettoyés: {self.stats['whitespace_cleaned']}")
